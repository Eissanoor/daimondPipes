...
if(quoteForm){quoteForm.addEventListener('submit',e=>{e.preventDefault();const fd=new FormData(quoteForm);const msg=`Quotation Request - Diamond PVC%0A%0AName: ${encodeURIComponent(fd.get('name'))}%0APhone: ${encodeURIComponent(fd.get('phone'))}%0ACompany: ${encodeURIComponent(fd.get('company')||'N/A')}%0AProduct: ${encodeURIComponent(fd.get('product'))}%0AQuantity/Size: ${encodeURIComponent(fd.get('quantity')||'N/A')}%0ALocation: ${encodeURIComponent(fd.get('location')||'N/A')}%0AMessage: ${encodeURIComponent(fd.get('message')||'')}`;window.open(`https://wa.me/923339557000?text=${msg}`,'_blank')})}
